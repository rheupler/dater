$(document).ready(function() {
	 ...
	 if(user!="")	 
	 {
	  ...
	  //changing css for base64 for supported for IE.
		 ...
		 if(msieversion() ||  navigator.userAgent.toLowerCase().indexOf('firefox') > -1){
	...
	new AjaxUpload($('#' + btnID), {
		...
		onSubmit : function(file, ext1) {
			...
			if (fileExt == "Excel") {
				...
				openErrorDialog("Selected file is not " + fileExt + " file");
				return false;
			} else if (fileExt == "any") {
...
function loginwithfb() {
	...
	showProgress();
	/*
	 * $.ajax({ crossDomain: true, url :"/hauth/login/Facebook",
	 * dataType:"text", success : function(response) {
	 * 
	 * 
	 * if(response=="success"){ location.reload(); } else { var msg="Failed to
	 * Login, Pls Try Again"; login(msg); } }, error : function(e, s, a) {
	 * 
	 * openErrorDialog("Failed to Login, Pls Try Again"); } });
	 */
}
...
function createLinkView(response) {
	...
	$.each(response, function(i, value) {
		...
		if (value.viewname != 0) {
			...
			data += "<ul>";
			data += "<li><b>Link : </b><a href=" + value.link
					+ ">http://codebeautify.org/" + value.link + "</a></li>";
			...
			if (value.user_id != "0") {
				...
				data += "<a href='#' onclick=editSavelink('" + i
						+ "') style='padding-right: 6px;'>Edit</a>";
				data += "<a href='#' onclick='deleteSavelink(" + value.id
						+ ")'>Delete</a>";
				data += "</div>";
			}
...
function checkRank(){
	...
	else if($('#keywords').val() == null || $('#keywords').val().length == 0){
		openErrorDialog("Please Enter keywords");
		return false;
	}
	...
	$('#results').html('Please wait while we search...it may take up to 60 seconds to complete your search.');
	$.post('Googlesearch',{d:url,k:$('#keywords').val(),c:country,dataType:'json'},function(data){
		
		if(data == 'googleFailed'){
			$("#results").html("Unable to reach google at this time, please try again later.");
			return false;
		}
		else if(data == 'domainFailed'){
			var msg = "We could not find the domain name " + url + " please verify the name.";
			$("#results").html(msg);
			return false;
		}
		...
		var arr = [],i = 0,index = 1,rank = 0;
		if(parseInt(d.spot) == d.spot){
			rank = parseInt(d.spot);
		}
		if(getUrlFromString(d.previous) != '--'){
			
			if(rank != 0){
				rank = rank - 1;
			}
			
			arr[i] = [index++,getUrlFromString(d.previous),$('#keywords').val(),d.total_search,rank,d.page];
			i++;
		}
		arr[i] = [index++,url,$('#keywords').val(),d.total_search,d.spot,d.page];
		i++;
		if(getUrlFromString(d.next) != '--'){
			
			if(rank != 0 && getUrlFromString(d.previous) != '--'){
				rank = rank + 2;
			}
			else{
				rank = parseInt(d.spot) + 1;
			}
			
			arr[i] = [index++,getUrlFromString(d.next),$('#keywords').val(),d.total_search,rank,d.page];
		}
		
		$("#hResult").show();
		
		getHistoryData(0,url);
		
		$('#keyWordResult').dataTable({
			"bRetrieve" : false,
			"bDestroy" : true,
			"bPaginate" : false,
			"bFilter" : false,
		...
		i = i - 1;
		$('#keyWordResult').find('tbody tr:eq('+(i)+')').addClass('chnage_green');
		
		$('#results').hide();
		
		 $.ajax({
				url :"/service/saveKeywordHistory",
			    type:'post',
			    data : {
			    	...
			    	spot : d.spot,
			    	page : d.page,
			    	previous : getUrlFromString(d.previous),
			    	next : getUrlFromString(d.next),
			    	country : 'india'
			    },
				success : function(response) {
				}
			...
			$('#results').html('The server returned an error, please try again later.');
	});
}

function getHistoryData(page,domain){
	
	 $.ajax({
			url :"/service/getKeywordHistory",
		    type:'post',
		    data : {
		    	page : page,
		    	domain : domain
			...
			success : function(response) {
				...
				$.each($.parseJSON(response),function(i,value){
					arr[i] = [(i+1),value.keywords,value.spot,value.page,value.pre_compititor,value.next_compititor]
				});
				
				showHistoryTable(arr);
			}
	 });
}

function checkRankOnenter(e){
	var key = e.which;
...
function showHistoryTable(arr){
	$("#hResult1").show();
	$('#historyResult').dataTable({
		"bRetrieve" : false,
		"bDestroy" : true,
		"bPaginate" : true,
		"bJQueryUI" : true,
		...
		"aoColumns" : [ {
			...
			"sTitle" : "Previous Compititer",
			"bVisible" : true,

		}, {
			"sTitle" : "Next Compititor",
			"bVisible" : true
		} ]
	});
...
function jsonTocsvbyjson(data,returnFlag) {
	...
	try {
		...
		if(returnFlag == undefined || !returnFlag){
			editorResult.setValue("Error in Convert to CSV");
		}
		else{
			htmlOutput("Error in Convert : add proper input format");
		}
		return false;
	}
